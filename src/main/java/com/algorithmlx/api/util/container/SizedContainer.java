...
import com.algorithmlx.api.util.tools.ContainerTag;
import com.algorithmlx.api.util.tools.helper.Scale;
...
public class SizedContainer extends AdvancedContainerMenu {
    private final Player player;
    private final InteractionHand hand;
    int padding = 8;
    int titleSpace = 10;
    public SizedContainer(int windowId, Inventory playerInventory, InteractionHand hand) {
        super(LVRegister.SC.get(), windowId, null, playerInventory);
    ...
    private void start(Inventory inventory, ItemStack itemStack) {
        ...
        var type = this.getDynamicItem().getBuilder();
    ...
    public Scale.Size getSize() {
        var type = getDynamicItem().getBuilder();
        return new Scale.Size(padding * 2 + Math.max(type.getRowWidth(), 9) * 18, padding * 2 + titleSpace * 2 + 8 + (type.getRowHeight() + 4) * 18);
    }

    public Scale.Point getBackpackSlots(Scale.Size size, int x, int y) {
        var type = getDynamicItem().getBuilder();
        return new Scale.Point(size.width() / 2 - type.getRowWidth() * 9 + x * 18, padding + titleSpace + y * 18);
    }

    public Scale.Point getPlayerSlots(Scale.Size size, int x, int y) {
        return new Scale.Point(size.width() / 2 - 9 * 9 + x * 18, size.height() - padding - 4 * 18 - 3 + y * 18 + (y == 3 ? 4 : 0));
    }
