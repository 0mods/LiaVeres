package com.algorithmlx.api.gltf.models

import com.algorithmlx.api.util.JUtils
...
internal class Vector4Deserializer: JsonDeserializer<Vector4f> {
    override fun deserialize(json: JsonElement, typeOfT: Type?, context: JsonDeserializationContext?): Vector4f {
        ...
        return Vector4f(array[0].asFloat, array[1].asFloat, array[2].asFloat, array[3].asFloat)
    }
}
...
object GltfDefinition {
    @JvmStatic
    var model = reloc("models/entity/npc.geo.gltf")
    ...
    fun main(args: Array<String>) {
        val modelLocation = model
        val file = parse(JUtils.getResource(modelLocation))
